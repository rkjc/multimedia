...
2) - The user can elect to use the java file chooser GUI. If the program is being run from Eclipse, or possibly some other IDEs, the GUI file chooser window will open behind the Eclipse window. The Eclipse window will have to be moved to reveal the file chooser.
...
1)   Version 1 is exactly as the homework assignment described. 
...
2)   Version-2 is different from version-1 only in the way that it converts from an 8x8 block to a sequence using the zigzag translator. Instead of first going right and then diagonal down to the left the alternate zigzag starts by first going down then diagonal up to the right. 
...
This not how any of the examples in the lectures showed the zigzag pattern working.
...
Version-2 does not completely match the Compression Ratio values given for Ducky.ppm.
The Chroma Cb and Cr cost values match exactly, but the Luma values don't match and therefore the Compression Ratio values doesn't match.

 
*******
notes:
I am fairly sure that the zigzag order used to compress the images in the sample data has an error in it. I think it is zigging where it should have zagged. 
...
The DCT algorithm that we are using is specifically for encoding values in the YCbCr [-128,127] range. 

I would like to humbly recommend that in the future you might provide an example that includes all the steps that we are using. Especially including the translation from RGB to YCbCr. 

However, not having a complete example spoon fed to us made it so that I really had to dig into the process to understand why it wasn't working. Same goes for the problem with the zigzag order.
