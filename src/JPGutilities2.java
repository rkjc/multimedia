...
public class JPGutilities2 {
	...
	public static void processJPG(File file) {
		...
		float[][][] origImgArray = imageRGBtoDoubleArray(origImg);
		
		// Pad Array size to multiple of 8
		float[][][] padArrayRGB = padImageArray(origImgArray, padX, padY);
		
		// Transform each pixel	from RGB to YCbCr		
		float[][][] arrayYCbCr = new float[sizeX][sizeY][3];
		...
		float[][] arrayY = new float[sizeX][sizeY];
		float[][] arrayCb = new float[sizeX][sizeY];
		float[][] arrayCr = new float[sizeX][sizeY];
		for(int y = 0; y < sizeY; y++){
			for(int x = 0; x < sizeX; x++){	
		...
		float[][] subSampleCb = new float[chromaSizeX][chromaSizeY];
		float[][] subSampleCr = new float[chromaSizeX][chromaSizeY];	
		for(int y = 0; y < chromaSizeY; y++){	// Initialize empty array to all 0's
			for(int x = 0; x < chromaSizeX; x++){	
				subSampleCb[x][y] = 0.0f;
				subSampleCr[x][y] = 0.0f;
			}
		}
		for(int y = 0; y < sizeY; y += 2){
			for(int x = 0; x < sizeX; x += 2){
				subSampleCb[x/2][y/2] = (arrayCb[x][y] + arrayCb[x + 1][y] + arrayCb[x][y + 1] + arrayCb[x + 1][y + 1]) / 4.0f;
				subSampleCr[x/2][y/2] = (arrayCr[x][y] + arrayCr[x + 1][y] + arrayCr[x][y + 1] + arrayCr[x + 1][y + 1]) / 4.0f;
			}
		}
					
		// Perform the DCT for Y image, Cb image, and Cr image 
		float[][] YarrayDCT = runDCTconvertion(arrayY);
		float[][] CbArrayDCT = runDCTconvertion(subSampleCb);
		float[][] CrArrayDCT = runDCTconvertion(subSampleCr);
		
		// Define for use outside the compression ratio output loop
		float[][] quantizedY = new float [sizeX][sizeY];
		float[][] quantizedCb = new float [chromaSizeX][chromaSizeY];
		float[][] quantizedCr = new float [chromaSizeX][chromaSizeY];
		...
		for(quality = 0; quality < 6; quality++){	//loop quantization values [0 - 5]
			...
			for(int xx = 0; xx < sizeX; xx += 8){
				for(int yy = 0; yy < sizeY; yy += 8){
					...
					float[] sequence = blockToSequence(block);
		...
		// Undo all the conversions
			...
			float[][] deQuantizedY = deQuantizeLuma(quantizedY, quality);
			float[][] deQuantizedCb = deQuantizeChroma(quantizedCb, quality);
			float[][] deQuantizedCr = deQuantizeChroma(quantizedCr, quality);
			
			// Undo the DCT for Cb image and Cr image and the Y image
			float[][] invDctArrayY = inverseDCTconvertion(deQuantizedY);
			float[][] invDctArrayCb = inverseDCTconvertion(deQuantizedCb);
			float[][] invDctArrayCr = inverseDCTconvertion(deQuantizedCr);
			
			// Supersample prep
			float[][] outArrayY = new float[sizeX][sizeY];
			float[][] outArrayCb = new float[sizeX][sizeY];
			float[][] outArrayCr = new float[sizeX][sizeY];
			...
			float[][][] outYCbCrArray = new float[sizeX][sizeY][3];
			for(int y = 0; y < sizeY; y++){
				for(int x = 0; x < sizeX; x++){	
			...
			float[][][] outPadImgArrayRGB = new float[sizeX][sizeY][3];
			...
			// Remove the padding
			float[][][] outImgArrayRGB = unPadImageArray(outPadImgArrayRGB, padX, padY);
...
//@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	public static float[][][] imageRGBtoDoubleArray(Image img) {
		int imgX = img.getW();
		int imgY = img.getH();
		float[][][] imgArray = new float[imgX][imgY][3];
		int[] rgb = new int[3];
	...
	public static Image arrayRGBtoImage (float[][][] imgArray) {
	...
	public static float[][][] padImageArray(float[][][] origImg, int padX, int padY) {
		int orgX = origImg.length;
		int orgY = origImg[0].length;
		int padWX = orgX + padX;
		int padHY = orgY + padY;
		float[][][] padImg = new float[padWX][padHY][3];
	...
	public static float[][][] unPadImageArray(float[][][] origImg, int padX, int padY) {
		int orgX = origImg.length;
		int orgY = origImg[0].length;
		int padWX = orgX - padX;
		int padHY = orgY - padY;
		float[][][] unPadImg = new float[padWX][padHY][3];
		for(int y = 0; y < padHY; y++){
			for(int x = 0; x < padWX; x++){
	...
	public static float[] matrixRGBtoYCbCR(float[] rgb) {
		float Y = 0.0f; float Cb = 0.0f; float Cr = 0.0f;
		float R = (float)rgb[0]; float G = (float)rgb[1]; float B = (float)rgb[2];
		
		Y = R*(0.2990f) + G*(0.5870f) + B*(0.1140f);
		Cb = R*(-0.168736f) + G*(-0.331264f) + B*(0.5000f);
		Cr = R*(0.5000f) + G*(-0.418688f) + B*(-0.081312f);
		
		Y = Y - 128.0f;
		Cb = Cb - 0.5f;
		Cr = Cr - 0.5f;
		
		Y = Math.max(Math.min(Y, 127.0f), -128.0f);
		Cb = Math.max(Math.min(Cb, 127.0f), -128.0f);
		Cr = Math.max(Math.min(Cr, 127.0f), -128.0f);
	
		return new float[] {Y, Cb, Cr};
	}
	
	public static float[] matrixYCbCRtoRGB(float[] ycbcr) {
		float R = 0.0f; float G = 0.0f; float B = 0.0f;
		float Y = ycbcr[0]; float Cb = ycbcr[1]; float Cr = ycbcr[2];
		Y = Y + 128.0f;
		Cb = Cb + 0.5f;
		Cr = Cr + 0.5f;
		
		R = Y*(1.0000f) + Cb*(0.0f) + Cr*(1.4020f);
		G = Y*(1.0000f) + Cb*(-0.34414f) + Cr*(-0.71414f);
		B = Y*(1.0000f) + Cb*(1.7720f) + Cr*(0.0f);
	...
	public static float[][] runDCTconvertion(float[][] f) {			
		int sizeX = f.length;
		int sizeY = f[0].length;
		float[][] F = new float[sizeX][sizeY];
		int u, v, x, y;
		for(int xx = 0; xx < sizeX; xx += 8){
			for(int yy = 0; yy < sizeY; yy += 8){
				...
				for(int u1 = xx; u1 < xx + 8; u1++){
					...
					for(int v1 = yy; v1 < yy + 8; v1++){
						...
						F[u1][v1] = Math.max(Math.min(((Cfunc(u) * Cfunc(v)) / 4.0f) * subSum, 1024.0f), -1024.0f);
					}
				}	
			}
		}	
	...
	public static float[][] inverseDCTconvertion(float[][] Fp) {	// inverses DCT on 8x8 array blocks	
		int sizeX = Fp.length;
		int sizeY = Fp[0].length;
		float[][] fp = new float[sizeX][sizeY];
		int u, v, x, y;
	...
	public static float Cfunc(float x) {
		if(x == 0)
	...
	public static float[][] quantizeLuma(float[][] input, int n){
		int x, y;
		int sizeX = input.length;
		int sizeY = input[0].length;
		float[][] output = new float[sizeX][sizeY];
		float[][] YquantTable = getYquantTable();
	...
	public static float[][] deQuantizeLuma(float[][] input, int n){
		int x, y;
		int sizeX = input.length;
		int sizeY = input[0].length;
		float[][] output = new float[sizeX][sizeY];
	...
	public static float[][] quantizeChroma(float[][] input, int n){
		int x, y;
		int sizeX = input.length;
		int sizeY = input[0].length;
		float[][] output = new float[sizeX][sizeY];
		float[][] CromeQuant = getCromeQuant();
	...
	public static float[][] deQuantizeChroma(float[][] input, int n){
		int x, y;
		int sizeX = input.length;
		int sizeY = input[0].length;
		float[][] output = new float[sizeX][sizeY];
			...
			for(int yy = 0; yy < sizeY; yy += 8){
				...
				for(int x1 = xx; x1 < xx + 8; x1++){
					...
					for(int y1 = yy; y1 < yy + 8; y1++){
						...
						output[x1][y1] = (float) (input[x1][y1] * (CromeQuant[x][y] * Math.pow(2.0f, n)));
					}
				}
			}
		}
	...
	public static float runLengthEncode(float[] inSequence){
		int pairCount = 0;
		float[][] RLEpairs = new float[64][2];
		float preValue = inSequence[1];  
		//float preValue = inSequence[0];   //breaks the code - make redblack work
		int cc = 1;
		int i = 0;
		for(i = 2; i < 64; i++){
	...
	public static float[] blockToSequence(float[][] input) {
	...
	public static float[][] getYquantTable() {
float[][] table = new float[8][8];
	...
	public static float[][] getCromeQuant() {
float[][] table = new float[8][8];
